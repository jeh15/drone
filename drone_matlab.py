...
class Drone_Risk(object):
    ...
    """
    matlab = __import__('matlab.engine')
    ...
    def generate_trajectory(self):
        ...
        # MATLAB QUADPROG:
        self.eng.workspace['H'] = self.matlab.double(self.H.toarray().tolist())
        self.eng.workspace['f'] = self.matlab.double(self.q.tolist())
        _A = self.numpy.vstack((self.A.toarray(), -self.A.toarray()))
        self.eng.workspace['A'] = self.matlab.double(_A.tolist())
        _b = self.numpy.concatenate((self.u, -self.l))
        self.eng.workspace['b'] = self.matlab.double(_b.tolist())
        self.qp_results = self.eng.quadprog(self.eng.workspace['H'], self.eng.workspace['f'], self.eng.workspace['A'], self.eng.workspace['b'], nargout=4)
    ...
    def get_fpf(self):
        ...
        # QUADPROG:
        self.eng.workspace['H_fpf'] = self.matlab.double(self._H_fpf.tolist())
        self.eng.workspace['f_fpf'] = self.matlab.double(self._f_fpf.tolist())
        _A = self.numpy.vstack((self._A_fpf.toarray(), -self._A_fpf.toarray()))
        self.eng.workspace['A_fpf'] = self.matlab.double(_A.tolist())
        _b = self.numpy.concatenate((self._u_fpf, -self._l_fpf))
        self.eng.workspace['b_fpf'] = self.matlab.double(_b.tolist())
        self.fpf_results = self.eng.quadprog(self.eng.workspace['H_fpf'], self.eng.workspace['f_fpf'], self.eng.workspace['A_fpf'],
                                            self.eng.workspace['b_fpf'], nargout=4)
        self.fpf_y = self.numpy.asarray(self.fpf_results[0], dtype=float)
    ...
    def initialize_fpf(self):
        ...
        self._A_fpf = self.scipy.sparse.eye(self.spline_resolution + 1, format='csc')
        self._l_fpf = self.numpy.zeros((self.spline_resolution + 1,), dtype=float)
        self._u_fpf = self.numpy.ones((self.spline_resolution + 1,), dtype=float)
    ...
    def get_ls(self):
        ...
        # QUADPROG:
        self.eng.workspace['H_ls'] = self.matlab.double(self._H_ls.tolist())
        self.eng.workspace['f_ls'] = self.matlab.double(self._f_ls.tolist())
        _A_ls = self.numpy.vstack((self._A.toarray(), -self._A.toarray()))
        self.eng.workspace['A_ls'] = self.matlab.double(_A_ls.tolist())
        _b = self.numpy.concatenate((self._u_ls, -self._l_ls))
        self.eng.workspace['b_ls'] = self.matlab.double(_b.tolist())
        self.ls_results = self.eng.quadprog(self.eng.workspace['H_ls'], self.eng.workspace['f_ls'],
                                            self.eng.workspace['A_ls'],
                                            self.eng.workspace['b_ls'], nargout=4)
        self.ls_y = self.numpy.asarray(self.ls_results[0], dtype=float)
    ...
    def initialize_ls(self):
        ...
        self._l_ls = -100.0 * self.numpy.ones((self.spline_resolution + 1 + self.spline_resolution - 1,), dtype=float)
        self._u_ls = self.numpy.zeros((self.spline_resolution + 1 + self.spline_resolution - 1,), dtype=float)
